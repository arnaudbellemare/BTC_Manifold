...
if df is not None and len(df) > 10:
    ...
    times = (times_pd - times_pd.iloc[0]).dt.total_seconds() / (24 * 3600)  # Changed to days
...
Yes.
        ...
        with col1:
            ...
            if np.isnan(price_std) or np.isnan(price_mean):
                ...
                # Downsample paths to 100 points per path to stay under Altair's row limit
                num_points = min(100, N)
                indices = np.linspace(0, N-1, num_points, dtype=int)
                path_data = [{"Time": t[j], "Price": paths[i, j], "Path": f"Path_{i}"} 
                             for i in range(min(n_paths, n_display_paths)) for j in indices]
                ...
                if plot_df.empty or plot_df[['Time', 'Price']].isna().any().any():
                    ...
                    base = alt.Chart(plot_df).encode(
                        x=alt.X("Time:Q", title="Time (days)"),  # Updated label
